...
// InitConfig returns the default initial MG configuration
...
func SetApimConfigs(client *client.Client, mgConfigs *Configuration) error {
	...
	mgConfigs.VerifyHostname = apimConfig.Data[verifyHostnameConst]
	mgConfigs.EnabledGlobalTMEventPublishing = apimConfig.Data[enabledGlobalTMEventPublishingConst]
	mgConfigs.JmsConnectionProvider = apimConfig.Data[jmsConnectionProviderConst]
	mgConfigs.ThrottleEndpoint = apimConfig.Data[throttleEndpointConst]
	mgConfigs.ApimUsername = string(apimSecret.Data["username"])
	mgConfigs.ApimPassword = string(apimSecret.Data["password"])
	mgConfigs.ThrottlingRecieverURL = apimConfig.Data[throttlingRecieverURLConst]
	mgConfigs.ThrottlingAuthURL = apimConfig.Data[throttlingAuthURLConst]
	mgConfigs.EnableRealtimeMessageRetrieval = apimConfig.Data[enableRealtimeMessageRetrievalConst]
	mgConfigs.EnableRequestValidation = apimConfig.Data[enableRequestValidationConst]
	mgConfigs.EnableResponseValidation = apimConfig.Data[enableResponseValidationConst]
	mgConfigs.EnabledEventhub = apimConfig.Data[enabledEventhub]
	mgConfigs.LogLevel = apimConfig.Data[logLevelConst]
	mgConfigs.EnabledAPIKeyIssuer = apimConfig.Data[enabledAPIKeyIssuerConst]
	mgConfigs.APIKeyKeystorePath = apimConfig.Data[apiKeyKeystorePathConst]
	mgConfigs.APIKeyKeystorePassword = apimConfig.Data[apiKeyKeystorePasswordConst]
	mgConfigs.APIKeyIssuerName = apimConfig.Data[apiKeyIssuerNameConst]
	mgConfigs.APIKeyIssuerCertificateAlias = apimConfig.Data[apiKeyIssuerCertificateAliasConst]
	...
	if err != nil {
		...
		mgConfigs.ValidityTime = validityTimeValConst
	} else {
		mgConfigs.ValidityTime = int32(validityTime)
	}
	...
	mgConfigs.APIKeyAllowedAPIs = apiKeyTokenIssuerAPIs
	...
	if err != nil {
		...
		mgConfigs.HttpPort = httpPortValConst
	} else {
		mgConfigs.HttpPort = int32(httpPort)
	}
	...
	if err != nil {
		...
		mgConfigs.HttpsPort = httpsPortValConst
	} else {
		mgConfigs.HttpsPort = int32(httpsPort)
	}
	mgConfigs.JwtHeader = apimConfig.Data[jwtHeaderConst]
	mgConfigs.EnabledJwtGenerator = apimConfig.Data[enabledJwtGeneratorConst]
	mgConfigs.JwtClaimDialect = apimConfig.Data[jwtClaimDialectConst]
	mgConfigs.JwtSigningAlgorithm = apimConfig.Data[jwtSigningAlgorithmConst]
	mgConfigs.JwtCertificateAlias = apimConfig.Data[jwtCertificateAliasConst]
	mgConfigs.JwtPrivateKeyAlias = apimConfig.Data[jwtPrivateKeyAliasConst]
	...
	if err != nil {
		...
		mgConfigs.JwtTokenExpiry = jwtTokenExpiryValConst
	} else {
		mgConfigs.JwtTokenExpiry = int32(jwtTokenExpiry)
	}
	mgConfigs.JwtIssuer = apimConfig.Data[jwtIssuerConst]
	mgConfigs.JwtGeneratorImpl = apimConfig.Data[jwtGeneratorImplConst]
	mgConfigs.JwtTokenCacheEnable = apimConfig.Data[jwtTokenCacheEnableConst]
	...
	if err != nil {
		logConf.Error(err, "Provided JWT Token cache expiry time is invalid. Using the default JWT token cache"+
			"expiry time")
		mgConfigs.JwtTokenCacheExpiryTime = jwtTokenCacheExpiryTimeValConst
	} else {
		mgConfigs.JwtTokenCacheExpiryTime = int32(jwtTokenCacheExpiryTime)
	}
	...
	mgConfigs.JwtRestrictedClaims = jwtRestrictedClaims
	...
	mgConfigs.JwtAudience = jwtAudience
	...
	if err != nil {
		logConf.Error(err, "Provided JWT Token cache capacity is invalid. Using the default JWT token cache"+
			"capacity")
		mgConfigs.JwtTokenCacheCapacity = jwtTokenCacheCapacityValConst
	} else {
		mgConfigs.JwtTokenCacheCapacity = int32(jwtTokenCacheCapacity)
	}
	...
	if err != nil {
		logConf.Error(err, "Provided JWT Token cache eviction factor is invalid. Using the default JWT token cache"+
			"eviction factor")
		mgConfigs.JwtTokenCacheEvictionFactor = jwtTokenCacheEvictionFactorValConst
	} else {
		mgConfigs.JwtTokenCacheEvictionFactor = jwtTokenCacheEvictionFactor
	}

	mgConfigs.TracingEnabled = strings.EqualFold(apimConfig.Data[tracingEnabledConst], "true")
	if mgConfigs.TracingEnabled {
		jaegerPort, err := strconv.Atoi(apimConfig.Data[tracingJaegerPortConst])
		if err != nil {
			logConf.Error(err, "Provided jaeger port is not valid.")
		} else {
			mgConfigs.TracingJaegerPort = int32(jaegerPort)
		}
		mgConfigs.TracingJaegerHost = apimConfig.Data[tracingJaegerHostConst]
	}
	return nil
}

// ApplyConfFile render and add the MGW configuration file to cluster
func ApplyConfFile(client *client.Client, userNamespace, apiName string, kanikoProps *kaniko.JobProperties, mgConfigs *Configuration, owner *[]metav1.OwnerReference) error {
	...
	finalConf, errRender := str.RenderTemplate(templateText, mgConfigs)
	if errRender != nil {
