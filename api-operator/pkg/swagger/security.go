...
func GetSecurityMap(apiSwagger *openapi3.Swagger) (map[string][]string, bool, int, error) {
	...
	if isDefined {
		logger.Info("API level security is defined")
		rawmsg := apiLevelSecurity.(json.RawMessage)
		errsec := json.Unmarshal(rawmsg, &APILevelSecurity)
		if errsec != nil {
			logger.Error(errsec, "error unmarshaling API level security ")
		...
		logger.Info("API Level security is not defined")
	}
	...
	if resSecIsDefined {
		...
		if errSec != nil {
			logger.Error(errSec, "error unmarshal into resource level security")
			return securityMap, isDefined, len(securityDef.Security), errSec
		}
	...
	if len(securityDef.Security) > 0 {
		logger.Info("Resource level security is defined")
		...
		logger.Info("Resource level security is not defined")
	}
